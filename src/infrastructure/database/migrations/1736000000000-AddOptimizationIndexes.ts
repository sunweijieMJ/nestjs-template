...
export class AddOptimizationIndexes1736000000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`CREATE INDEX "IDX_favorite_userId_targetType" ON "favorite" ("userId", "targetType")`);

    // Composite index for address sorting (userId + isDefault + createdAt)
    await queryRunner.query(
      `CREATE INDEX "IDX_address_userId_isDefault_createdAt" ON "address" ("userId", "isDefault" DESC, "createdAt" DESC)`,
    );

    // Soft delete indexes for better query performance
    await queryRunner.query(`CREATE INDEX "IDX_user_deletedAt" ON "user" ("deletedAt") WHERE "deletedAt" IS NULL`);

    await queryRunner.query(
      `CREATE INDEX "IDX_address_deletedAt" ON "address" ("deletedAt") WHERE "deletedAt" IS NULL`,
    );

    await queryRunner.query(
      `CREATE INDEX "IDX_session_deletedAt" ON "session" ("deletedAt") WHERE "deletedAt" IS NULL`,
    );
  }
