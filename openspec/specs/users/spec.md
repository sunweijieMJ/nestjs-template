...
用户管理模块，提供完整的用户 CRUD（创建、读取、更新、删除）操作接口。所有接口仅限管理员角色 (admin) 访问。支持分页查询、条件筛选和排序。用户数据包括基本信息、角色、状态、头像等。采用六边形架构，支持 PostgreSQL 和 MongoDB 双数据库。
...
| 属性 | 类型 | 说明 |
|------|------|------|
| id | number \| string | 主键 (PostgreSQL 为 number，MongoDB 为 string) |
