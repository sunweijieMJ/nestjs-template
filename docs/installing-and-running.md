...
1. Run additional container:
   ...
   ```bash
   docker compose -f docker/docker-compose.base.yaml -f docker/docker-compose.mongo.yaml up -d mongo mongo-express maildev
   ```
...
1. Run containers
   ...
   ```bash
   npm run docker:pg:dev
   ```

1. For check status run

   ```bash
   docker compose -f docker/docker-compose.base.yaml -f docker/docker-compose.postgres.yaml logs
   ```
...
1. For check status run
   ...
   ```bash
   docker compose -f docker/docker-compose.base.yaml -f docker/docker-compose.mongo.yaml logs
   ```
